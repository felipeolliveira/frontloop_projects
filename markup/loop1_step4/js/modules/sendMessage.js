export default function initSendMessage() {
  ...
  const classMessage = 'ballon-receive';
  
  const getTimeSendMessage = () => new Date();
  const getTimeSendMessageHours = () => getTimeSendMessage().getHours().toString().padStart(2, '0');
  const getTimeSendMessageMinutes = () => getTimeSendMessage().getMinutes().toString().padStart(2, '0');
